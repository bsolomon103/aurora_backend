def check_similarity(target, comparator):
    #target_phrase = 'start assessment'
    threshold = 0.999999999  # Adjust the threshold as needed

    # Remove whitespace and convert the input text to lowercase
    processed_text = comparator.strip().lower()
    ...
    for i in range(len(processed_text)):
        for j in range(i + 1, len(processed_text) + 1):
            ...
            distance = levenshtein_distance(target, substring)
            similarity = 1 - (distance / max(len(target), len(substring)))
