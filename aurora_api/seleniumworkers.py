...
class Driver:
  ...
  def login(self, url, idx, password, request):
   ...
   def getspecificlink(response: str) -> str:
       if response.__contains__('https://my.southend.gov.uk/'):
            if response.__contains__('Discounts, Disregard and Exemptions'):
                continue = WebDriverWait(self.driver,1).until(EC.visibility_of_element_located((By.XPATH, '/html/body/form/div[2]/main/div/div[3]/div/div/div[4]/a[1]')))
                continue.click()
                print(1)
...
class Driver:
    ...
    def getspecificlink(self, response: str) -> str:
        
        if response.__contains__('https://my.southend.gov.uk/'):
            if response.lower().__contains__('discount'):
                self.driver.get('https://my.southend.gov.uk/')
                continuum = WebDriverWait(self.driver,1).until(EC.visibility_of_element_located((By.XPATH, '/html/body/form/div[2]/main/div/div[3]/div/div/div[4]/a[1]/span')))
                continuum.click()
                return self.driver.current_url
                print(1)
                
                # Check for council tax
                menu = self.driver.find_element(By.XPATH, '/html/body/form/header/div[1]/div[1]/button')
                menu.click()
                print(2)
                print(self.driver.current_url)
                
                try:
                    council_tax = WebDriverWait(self.driver,1).until(EC.visibility_of_element_located((By.XPATH, "//a[text()='Council Tax']")))
                    council_tax.click()
